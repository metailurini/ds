...
collapsed to strings. Unknown element kinds remain unknown. Function parameters with literal defaults carry that default's static kind through the single lowered function body, so defaulted string parameters can use string methods; required untyped parameters remain unknown until typed parameters or a runtime type-tag design is deliberately added.
