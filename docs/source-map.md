...
| Mutable collections | accepted literals/indexing/push/array loops/map loops are HIR-backed; `src/lower_collection.c` owns portability gates; Bash push/map loops update or preserve type sidecars; unsupported assignment syntax is parser-rejected | do not add mutation AST/HIR without a parity contract |
