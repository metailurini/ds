...
static void emit_plain_command_fail_helper(BashEmitter *e) {
    ...
    buf_append(&e->out, "__ds_is_quiet_broken_pipe() { local __ds_code=$1; (( __ds_code == 141 )) && __ds_stdout_is_pipe_like; }\n");
    buf_append(&e->out, "__ds_fail() {\n");
    buf_append(&e->out, "  local __ds_loc=$1 __ds_code=$2\n");
    buf_append(&e->out, "  if __ds_is_quiet_broken_pipe \"$__ds_code\"; then exit 0; fi\n");
...
static void emit_cleanup_helpers(BashEmitter *e) {
    ...
    buf_append(&e->out, "__ds_stdout_is_pipe_like() { [[ -p /dev/stdout || -S /dev/stdout ]]; }\n");
    buf_append(&e->out, "__ds_is_quiet_broken_pipe() { local __ds_code=$1; (( __ds_code == 141 )) && __ds_stdout_is_pipe_like; }\n");
    buf_append(&e->out, "__ds_fail() { local __ds_loc=$1 __ds_code=$2; if __ds_is_quiet_broken_pipe \"$__ds_code\"; then if [[ \"${__ds_cleanup_running:-false}\" == true ]]; then return 0; fi; exit 0; fi; echo \"$__ds_loc: error: command failed with exit $__ds_code\" >&2; if [[ \"${__ds_cleanup_running:-false}\" == true ]]; then return \"$__ds_code\"; fi; exit \"$__ds_code\"; }\n");
    ...
    buf_appendf(&e->out, "__ds_run_pipeline() { local __ds_loc=$1 __ds_pipeline=$2 __ds_pid __ds_code; set +e; __ds_setsid bash -c '__ds_child_int(){ kill $(jobs -p) 2>/dev/null; wait 2>/dev/null; exit %d; }; __ds_child_term(){ kill $(jobs -p) 2>/dev/null; wait 2>/dev/null; exit %d; }; trap __ds_child_int %s; trap __ds_child_term %s; eval \"$1\" & wait \"$!\"' bash \"$__ds_pipeline\" & __ds_pid=$!; __ds_foreground_pid=$__ds_pid; trap '__ds_run_signal %s %d' %s; trap '__ds_run_signal %s %d' %s; wait \"$__ds_pid\"; __ds_code=$?; __ds_foreground_pid=; trap '__ds_run_signal %s %d' %s; trap '__ds_run_signal %s %d' %s; set -e; if (( __ds_code == %d )); then __ds_run_signal %s %d; fi; if (( __ds_code == %d )); then __ds_run_signal %s %d; fi; if __ds_is_quiet_broken_pipe \"$__ds_code\"; then if [[ \"${__ds_cleanup_running:-false}\" == true ]]; then return 0; fi; exit 0; fi; if (( __ds_code != 0 )); then printf '%%s: error: pipeline failed with exit %%s\\n' \"$__ds_loc\" \"$__ds_code\" >&2; return \"$__ds_code\"; fi; return 0; }\n",
