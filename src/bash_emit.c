...
static void emit_cleanup_helpers(BashEmitter *e) {
    buf_append(&e->out,
        ...
        "__ds_control_exit() { local __ds_loc=$1; shift; if (( $# != 1 )); then echo \"$__ds_loc: error: `exit` expects exactly one integer code\" >&2; if [[ \"${__ds_cleanup_running:-false}\" == true ]]; then return 1; fi; exit 1; fi; if [[ ! \"$1\" =~ ^[0-9]+$ ]] || (( $1 < 0 || $1 > 255 )); then echo \"$__ds_loc: error: `exit` code must be an integer from 0 to 255\" >&2; if [[ \"${__ds_cleanup_running:-false}\" == true ]]; then return 1; fi; exit 1; fi; if [[ \"${__ds_cleanup_running:-false}\" == true ]]; then __ds_handler_exit_requested=true; return \"$1\"; fi; exit \"$1\"; }\n"
        "__ds_run_stack_lifo() { local __ds_arr=$1 __ds_i __ds_fn __ds_code __ds_status=0; __ds_stack_exit_requested=false; __ds_stack_status=0; eval \"__ds_i=\\${#${__ds_arr}[@]}\"; while ((__ds_i > 0)); do __ds_i=$((__ds_i - 1)); eval \"__ds_fn=\\${${__ds_arr}[__ds_i]}\"; __ds_handler_exit_requested=false; set +e; \"$__ds_fn\"; __ds_code=$?; set -e; if [[ \"$__ds_handler_exit_requested\" == true ]]; then __ds_stack_exit_requested=true; __ds_status=$__ds_code; elif (( __ds_code != 0 )); then __ds_status=$__ds_code; fi; done; __ds_stack_status=$__ds_status; return \"$__ds_status\"; }\n"
        "__ds_run_cleanup() { local __ds_rc=${1:-0} __ds_code; if $__ds_cleanup_running; then exit \"$__ds_rc\"; fi; __ds_cleanup_running=true; trap - EXIT INT TERM; if [[ -n \"${__ds_trap_EXIT:-}\" ]]; then __ds_handler_exit_requested=false; set +e; \"$__ds_trap_EXIT\"; __ds_code=$?; set -e; if [[ \"$__ds_handler_exit_requested\" == true ]] || (( __ds_code != 0 )); then __ds_rc=$__ds_code; fi; fi; __ds_run_stack_lifo __ds_defer_EXIT; __ds_code=$?; if [[ \"$__ds_stack_exit_requested\" == true ]] || (( __ds_code != 0 )); then __ds_rc=$__ds_stack_status; fi; exit \"$__ds_rc\"; }\n"
        "__ds_run_signal() { local __ds_sig=$1 __ds_rc=$2 __ds_trap __ds_stack __ds_code; __ds_trap=__ds_trap_${__ds_sig}; __ds_stack=__ds_defer_${__ds_sig}; if $__ds_cleanup_running; then exit \"$__ds_rc\"; fi; __ds_cleanup_running=true; trap - EXIT INT TERM; local __ds_fn=\"${!__ds_trap:-}\"; if [[ -n \"$__ds_fn\" ]]; then __ds_handler_exit_requested=false; set +e; \"$__ds_fn\"; __ds_code=$?; set -e; if [[ \"$__ds_handler_exit_requested\" == true ]] || (( __ds_code != 0 )); then __ds_rc=$__ds_code; fi; fi; __ds_run_stack_lifo \"$__ds_stack\"; __ds_code=$?; if [[ \"$__ds_stack_exit_requested\" == true ]] || (( __ds_code != 0 )); then __ds_rc=$__ds_stack_status; fi; if [[ -n \"${__ds_trap_EXIT:-}\" ]]; then __ds_handler_exit_requested=false; set +e; \"$__ds_trap_EXIT\"; __ds_code=$?; set -e; if [[ \"$__ds_handler_exit_requested\" == true ]] || (( __ds_code != 0 )); then __ds_rc=$__ds_code; fi; fi; __ds_run_stack_lifo __ds_defer_EXIT; __ds_code=$?; if [[ \"$__ds_stack_exit_requested\" == true ]] || (( __ds_code != 0 )); then __ds_rc=$__ds_stack_status; fi; exit \"$__ds_rc\"; }\n"
        "__ds_run_direct_command() { local __ds_loc=$1; shift; local __ds_pid __ds_code; set +e; (trap - INT TERM; exec \"$@\") & __ds_pid=$!; trap '' INT TERM; wait \"$__ds_pid\"; __ds_code=$?; trap '__ds_run_signal INT 130' INT; trap '__ds_run_signal TERM 143' TERM; set -e; if (( __ds_code == 130 )); then __ds_run_signal INT 130; fi; if (( __ds_code == 143 )); then __ds_run_signal TERM 143; fi; if (( __ds_code != 0 )); then set +e; __ds_fail \"$__ds_loc\" \"$__ds_code\"; __ds_code=$?; return \"$__ds_code\"; fi; return 0; }\n"
        "__ds_run_pipeline() { local __ds_loc=$1 __ds_pipeline=$2 __ds_pid __ds_code; set +e; (trap - INT TERM; eval \"$__ds_pipeline\") & __ds_pid=$!; trap '' INT TERM; wait \"$__ds_pid\"; __ds_code=$?; trap '__ds_run_signal INT 130' INT; trap '__ds_run_signal TERM 143' TERM; set -e; if (( __ds_code == 130 )); then __ds_run_signal INT 130; fi; if (( __ds_code == 143 )); then __ds_run_signal TERM 143; fi; if (( __ds_code != 0 )); then printf '%s: error: pipeline failed with exit %s\\n' \"$__ds_loc\" \"$__ds_code\" >&2; return \"$__ds_code\"; fi; return 0; }\n"
