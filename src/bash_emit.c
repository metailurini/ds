...
static void emit_cleanup_helpers(BashEmitter *e) {
    buf_append(&e->out,
        ...
        "__ds_run_signal() { local __ds_sig=$1 __ds_rc=$2 __ds_trap=__ds_trap_${__ds_sig} __ds_stack=__ds_defer_${__ds_sig} __ds_code; if $__ds_cleanup_running; then exit \"$__ds_rc\"; fi; __ds_cleanup_running=true; trap - EXIT INT TERM; local __ds_fn=\"${!__ds_trap:-}\"; if [[ -n \"$__ds_fn\" ]]; then __ds_handler_exit_requested=false; set +e; \"$__ds_fn\"; __ds_code=$?; set -e; if [[ \"$__ds_handler_exit_requested\" == true ]] || (( __ds_code != 0 )); then __ds_rc=$__ds_code; fi; fi; __ds_run_stack_lifo \"$__ds_stack\"; __ds_code=$?; if [[ \"$__ds_stack_exit_requested\" == true ]] || (( __ds_code != 0 )); then __ds_rc=$__ds_stack_status; fi; if [[ -n \"${__ds_trap_EXIT:-}\" ]]; then __ds_handler_exit_requested=false; set +e; \"$__ds_trap_EXIT\"; __ds_code=$?; set -e; if [[ \"$__ds_handler_exit_requested\" == true ]] || (( __ds_code != 0 )); then __ds_rc=$__ds_code; fi; fi; __ds_run_stack_lifo __ds_defer_EXIT; __ds_code=$?; if [[ \"$__ds_stack_exit_requested\" == true ]] || (( __ds_code != 0 )); then __ds_rc=$__ds_stack_status; fi; exit \"$__ds_rc\"; }\n"
