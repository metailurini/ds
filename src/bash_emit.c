...
static void emit_cleanup_helpers(BashEmitter *e) {
    buf_append(&e->out,
        ...
        "__ds_run_pipeline() { local __ds_loc=$1 __ds_pipeline=$2 __ds_pid __ds_code; set +e; bash -c \"trap - INT TERM; $__ds_pipeline\" & __ds_pid=$!; trap '__ds_run_signal INT 130' INT; trap '__ds_run_signal TERM 143' TERM; wait \"$__ds_pid\"; __ds_code=$?; trap '__ds_run_signal INT 130' INT; trap '__ds_run_signal TERM 143' TERM; set -e; if (( __ds_code == 130 )); then __ds_run_signal INT 130; fi; if (( __ds_code == 143 )); then __ds_run_signal TERM 143; fi; if (( __ds_code != 0 )); then printf '%s: error: pipeline failed with exit %s\\n' \"$__ds_loc\" \"$__ds_code\" >&2; return \"$__ds_code\"; fi; return 0; }\n"
