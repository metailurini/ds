...
bool bash_emit_user_function_value_call_into(BashEmitter *e, DsStr name, const DsLowerExpr *call, int indent) {
    ...
    if (call->as.call.returns_row_array) {
        for (size_t i = 0; i < call->as.call.row_schema.len; i++) {
            emit_indent(&e->out, indent);
            bash_emit_row_field_array_name(&e->out, name, call->as.call.row_schema.items[i].name);
            buf_append(&e->out, "=(\"${");
            bash_emit_return_row_field_array_name(&e->out, call->as.call.row_schema.items[i].name);
            buf_append(&e->out, "[@]}\")\n");
        }
    }
    return true;
