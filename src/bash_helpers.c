...
const char *ds_bash_recursive_glob_helpers_source(void) {
    return
        ...
        "  local base=\"$1\"\n"
        "  find \"$base\" -type d \\( ! -path \"$base\" -name '.*' -prune -o -print \\)\n"
        "}\n"
        "__ds_glob_recursive() {\n"
        "  local pattern=\"$1\" prefix suffix base_pattern base dir glob_pat match strip_dot=false base_tmp dirs_tmp dirs_err matches_tmp\n"
        ...
        "  base_tmp=$(mktemp) || __ds_error 'failed to create recursive glob temp file'\n"
        "  dirs_tmp=$(mktemp) || { rm -f \"$base_tmp\"; __ds_error 'failed to create recursive glob temp file'; }\n"
        "  dirs_err=$(mktemp) || { rm -f \"$base_tmp\" \"$dirs_tmp\"; __ds_error 'failed to create recursive glob temp file'; }\n"
        "  matches_tmp=$(mktemp) || { rm -f \"$base_tmp\" \"$dirs_tmp\" \"$dirs_err\"; __ds_error 'failed to create recursive glob temp file'; }\n"
        "  { compgen -G \"$base_pattern\" || true; } >\"$base_tmp\"\n"
        "  while IFS= read -r base; do\n"
        "    local strip_match=\"$strip_dot\"\n"
        "    if [[ \"$base\" == -* ]]; then base=\"./$base\"; strip_match=true; fi\n"
        "    [[ -d \"$base\" && ! -L \"$base\" ]] || continue\n"
        "    if ! __ds_glob_recursive_dirs \"$base\" >\"$dirs_tmp\" 2>\"$dirs_err\"; then\n"
        "      local find_detail= find_re=\"^find: '(.*)': (.*)$\"\n"
        "      IFS= read -r find_detail <\"$dirs_err\" || true\n"
        "      if [[ \"$find_detail\" =~ $find_re ]]; then\n"
        "        find_detail=\"failed to traverse recursive glob directory '${BASH_REMATCH[1]}': ${BASH_REMATCH[2]}\"\n"
        "      else\n"
        "        find_detail=\"failed to traverse recursive glob directory '$base'\"\n"
        "      fi\n"
        "      rm -f \"$base_tmp\" \"$dirs_tmp\" \"$dirs_err\" \"$matches_tmp\"\n"
        "      __ds_error \"$find_detail\"\n"
        "    fi\n"
