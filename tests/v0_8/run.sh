...
assert_contains "$ROOT/src/ds_command_result.c" "ds_command_result_field_lookup" "command-result field lookup is centralized"
assert_contains "$ROOT/src/ds_command_result.c" '"status", "code"' "command-result status storage alias is centralized"
assert_contains "$ROOT/src/ds_command_result.c" "ds_command_result_field_count" "command-result field catalog exposes iteration"
...
assert_contains "$ROOT/src/bash_return.c" "bash_emit_array_return_payload" "Bash return emission delegates structured array payloads"
assert_contains "$ROOT/src/bash_return.c" "bash_emit_map_return_payload" "Bash return emission delegates structured map payloads"
assert_contains "$ROOT/src/bash_structured.c" "bash_emit_type_assignment_for_expr" "Bash structured ABI owns type-sidecar assignment helpers"
assert_contains "$ROOT/src/bash_function.c" "bash_emit_expr_type_value" "Bash function calls consume structured ABI type values"
assert_contains "$ROOT/src/ds_command_word.c" "ds_command_word_analyze" "command-word shape classification is centralized"
assert_contains "$ROOT/src/ds_command_word.c" "ds_command_word_contains_direct_call_interpolation" "direct call interpolation detection is centralized"
assert_contains "$ROOT/src/ds_command_pipeline.c" "ds_command_is_pipeline" "pipeline shape classification is centralized"
assert_contains "$ROOT/src/ds_command_pipeline.c" "ds_command_pipeline_status" "pipeline pipefail status policy is centralized"
...
assert_contains "$ROOT/src/lower_command.c" "ds_command_word_analyze" "lowerer consumes shared command-word classifier"
assert_contains "$ROOT/src/vm_process.c" "ds_command_word_analyze" "VM consumes shared command-word classifier"
assert_contains "$ROOT/src/vm_process.c" "ds_command_pipeline_status" "VM consumes shared pipeline status policy"
assert_contains "$ROOT/src/bash_command.c" "ds_command_word_analyze" "Bash consumes shared command-word classifier"
assert_contains "$ROOT/src/bash_stmt.c" "ds_command_is_pipeline" "Bash statement emission consumes shared pipeline classifier"
assert_contains "$ROOT/Makefile" "src/bash_function.c" "Bash function wrapper source is linked into ds"
assert_contains "$ROOT/Makefile" "src/bash_return.c" "Bash return payload source is linked into ds"
assert_contains "$ROOT/Makefile" "src/lower_collection.c" "collection portability policy is linked into ds"
assert_contains "$ROOT/src/lower_collection.c" "lower_validate_portable_collection_receiver" "collection receiver portability has a lowerer home"
assert_contains "$ROOT/src/lower_collection.c" "lower_validate_portable_collection_index" "collection index portability has a lowerer home"
assert_contains "$ROOT/src/lower_collection.c" "lower_collection_for_iterable_is_portable" "array iterable portability has a lowerer home"
assert_contains "$ROOT/src/lower_expr.c" "lower_validate_portable_collection_receiver" "expression lowering consumes collection receiver policy"
...
assert_contains "$ROOT/src/bash_emit.c" "ds_script_type_name" "Bash script help consumes shared script type labels"
assert_contains "$ROOT/src/vm_args.c" "ds_script_type_name" "VM script help consumes shared script type labels"
...
assert_contains "$ROOT/src/ds_checker.c" "ds_diag_report" "checker warnings use shared diagnostic rendering"
